...
router.get('/productos', async (req, res) => {
  try {
    const [productos] = await pool.query('SELECT id, name, price, line FROM products')
    res.json(productos)
  } catch (error) {
...
router.get('/productos-destacados', async (req, res) => {
  try {
    const [productos] = await pool.query('SELECT id, name, price, line FROM products LIMIT 3')
    res.json(productos)
  } catch (error) {
