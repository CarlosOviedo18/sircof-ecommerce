            ...
            <div className="flex items-center gap-4 md:gap-12 ml-auto">
              ...
              <div className="flex gap-3 md:gap-4 items-center">
                ...
                <button className="text-white hover:scale-110 transition-transform" aria-label="Buscar">
                  <svg xmlns="http://www.w3.org/2000/svg" width={20} height={20} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M3 10a7 7 0 1 0 14 0a7 7 0 1 0 -14 0" /><path d="M21 21l-6 -6" /></svg>
                </button>
            ...
            <div className="fixed inset-0 md:hidden z-40" onClick={() => setMobileMenuOpen(false)}>
              <nav className="absolute top-0 right-0 h-screen w-3/4 max-w-xs bg-gradient-to-b from-black/95 to-black/90 backdrop-blur-md shadow-2xl animate-in slide-in-from-right">
                ...
                <ul className="flex flex-col gap-1 font-syne px-6">
                  <li><a href="#inicio" className="text-white font-medium text-base hover:text-coffee transition-colors block py-3 px-4 rounded-lg hover:bg-white/10" onClick={() => setMobileMenuOpen(false)}>Inicio</a></li>
                  <li><a href="#paginas" className="text-white font-medium text-base hover:text-coffee transition-colors block py-3 px-4 rounded-lg hover:bg-white/10" onClick={() => setMobileMenuOpen(false)}>Sobre Nosotros</a></li>
                  <li><a href="#blog" className="text-white font-medium text-base hover:text-coffee transition-colors block py-3 px-4 rounded-lg hover:bg-white/10" onClick={() => setMobileMenuOpen(false)}>Blog</a></li>
                  <li><a href="#galeria" className="text-white font-medium text-base hover:text-coffee transition-colors block py-3 px-4 rounded-lg hover:bg-white/10" onClick={() => setMobileMenuOpen(false)}>Galería</a></li>
                  <li><a href="#tienda" className="text-white font-medium text-base hover:text-coffee transition-colors block py-3 px-4 rounded-lg hover:bg-white/10" onClick={() => setMobileMenuOpen(false)}>Tienda</a></li>
                </ul>
              </nav>
            </div>
          )}
