...
function AdminLayout() {
  ...
  const links = [
    { to: '/admin', label: t('sidebar.dashboard'), end: true, icon: (
    ...
    { to: '/admin/coffees', label: t('sidebar.coffees'), icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8h1a4 4 0 010 8h-1"/><path d="M2 8h16v9a4 4 0 01-4 4H6a4 4 0 01-4-4z"/><line x1="6" y1="1" x2="6" y2="4"/><line x1="10" y1="1" x2="10" y2="4"/><line x1="14" y1="1" x2="14" y2="4"/></svg>
    )},
    { to: '/admin/products', label: t('sidebar.products'), icon: (
    ...
    { to: '/admin/orders', label: t('sidebar.orders'), icon: (
    ...
    { to: '/admin/contacts', label: t('sidebar.contacts'), icon: (
    ...
    <div className="min-h-screen flex bg-gray-50">
      ...
      <aside className={`bg-[#1b2a4a] text-white flex flex-col fixed h-full transition-all duration-300 z-30 w-64 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0 md:w-[72px]'}`}>
        ...
        <div className="h-16 flex items-center px-5 border-b border-white/10 gap-3">
          ...
          {sidebarOpen && (
            <span className="text-lg font-bold tracking-wide"> {t('sidebar.cafeSircof')}</span>
          )}
        </div>
        ...
        <nav className="flex-1 py-4 overflow-y-auto">
          {sidebarOpen && (
            <div className="px-5 mb-3">
              <p className="text-[10px] uppercase tracking-widest text-gray-400 font-semibold">{t('sidebar.navigation')}</p>
            </div>
          )}
          {links.map(link => {
        ...
        <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-4 md:px-6 sticky top-0 z-10">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              ...
              <span className="hidden sm:inline">{t('sidebar.viewStore')}</span>
            </NavLink>
